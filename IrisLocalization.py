...
def Iris_localize(img):
    ...
    if distance > 15:
        outerCircle[0] = inner_x + np.finfo(np.float32).eps*(outerCircle[0]-inner_x) # add sgn(o-i) to preserve (some) directionality
        outerCircle[1] = inner_y + np.finfo(np.float32).eps*(outerCircle[1]-inner_y) 
